...
def load_encoder_from_checkpoint(model_path:str,
    ...
    # Filter out encoder parameters; this assumes encoder parameters are prefixed with 'encoder.'
    encoder_dict = {k: v for k, v in state_dict.items() if k.startswith('encoder.')}

    # Update the current model's state dict with encoder parameters
    model_state_dict = model.state_dict()
    model_state_dict.update(encoder_dict)
...
def freeze_encoder(model:pl.LightningModule):
    # Iterate through all parameters in the encoder and set requires_grad to False
...
def normal_train():
    ...
    autoencoder_checkpoint = "FullyAutoencoder_logs/beamforming_2024-03-28 18:50:12.425180/epoch=108-step=763.ckpt"
    model = load_encoder_from_checkpoint(autoencoder_checkpoint, model)

    # Freeze the encoder
    freeze_encoder(model)

    PATH = "FullyDecoder_logs"
...
def render():
    ...
    model = FullyEndToEnd.load_from_checkpoint(
        "FullyDecoder_logs/beamforming_2024-03-28 20:45:32.246514/epoch=11-step=4860.ckpt"
    )
    model = model.cuda()
...
if __name__ == "__main__":
    ...
    # normal_train()
    render()
# tensorboard --logdir FullyDecoder_logs
