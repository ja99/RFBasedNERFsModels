...
class FullyEndToEnd(pl.LightningModule):
    def __init__(self, n_antennas: int, n_timesteps: int, dim_position: int, dim_output: int,
        ...
        # ToDo: make encoder way bigger since n_antennas * n_timesteps * 5 is a lot of data
        # positional encoding?
        # encoding per t?
...
def normal_train():
    ...
    PATH = "FullySimplifiedSpereatedEncodedPerDir_logs"
...
def render():
    ...
    model = FullyEndToEnd.load_from_checkpoint(
        "FullySimplifiedSpereatedEncodedPerDir_logs/beamforming_2024-03-14 18:56:56.701377/epoch=10-step=4455.ckpt"
    )
    model = model.cuda()
    ...
    grid_values = grid_values.flatten(start_dim=1, end_dim=-2)
    ...
    ground_truth = grid_values[:, :, 1].view(original_shape)
...
# tensorboard --logdir FullySimplifiedSpereatedEncodedPerDir_logs
