...
class FullyEndToEnd(pl.LightningModule):
    def __init__(self, n_antennas: int, n_timesteps: int, dim_position: int, dim_output: int,
        ...
        self.decoder = nn.Sequential(
            nn.Linear(16*512, 8192),
            nn.ReLU(),
            nn.Linear(8192, 16*5*750),
            nn.ReLU(),
        )
    ...
    def training_step(self,
        ...
        loss = self.loss_fn(output, voltages.flatten(start_dim=1))
    ...
    def validation_step(self,
        ...
        print(f"{voltages.min()=} - {voltages.max()=}")
        ...
        if self.validation_step_counter >= 3:
            self.logger.experiment.add_histogram('actual_values', voltages.flatten(start_dim=1), self.global_step)
            self.logger.experiment.add_histogram('predicted_values', output, self.global_step)
...
if __name__ == "__main__":
    ...
    normal_train()
# tensorboard --logdir FullyAutoencoder_logs
